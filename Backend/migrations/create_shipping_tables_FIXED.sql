-- Migration: Create shipping configuration tables (FIXED VERSION)
-- Date: 2024-11-29
-- Description: Tables for store courier settings and courier services
-- Fix: Proper foreign key constraints with checks
...
-- 4. Master table for courier services (ekspedisi) - CREATE FIRST (no dependencies)
...
-- 5. Master table for courier service types - CREATE SECOND (depends on courier_services)
...
-- Note: Assumes 'stores' table exists with 'store_id' as INT PRIMARY KEY
...
-- This will fail gracefully if stores table doesn't exist or column doesn't match
...
-- Add store foreign key separately
...
SELECT COUNT(*) as courier_services_count FROM courier_services;
SELECT COUNT(*) as service_types_count FROM courier_service_types;
